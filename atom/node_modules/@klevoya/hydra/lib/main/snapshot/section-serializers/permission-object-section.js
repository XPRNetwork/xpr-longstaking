...
class PermissionObjectSerializer extends abstract_section_1.default {
    ...
    deserialize() {
        ...
        while (this.hasMoreRows()) {
            const row = this.readRow(`permission_object`);
            data.push(row);
        }
        this.data = data;
    }
    serialize() {
        ...
        for (const row of rows) {
            this.serializeRow(buf, `permission_object`, row);
        }
        return Buffer.from(buf.asUint8Array());
    }
}
exports.default = PermissionObjectSerializer;
